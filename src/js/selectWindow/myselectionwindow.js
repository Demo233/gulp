...
function MySelectionWindow(viewer, options) {
    ...
    var _container_bounding_top = _container.getBoundingClientRect().top; // _container距离顶部的距离
    var _container_bounding_left = _container.getBoundingClientRect().left; // _container距离左边的距离

    var _glob_idsArr = []; // 选中构件数组arr
    var _currentKeyCode = null; // 当前键盘按中keyCode
     ...
     * @param {flag} flag 
     */
    this.handleSelect = function(flag) {
      _currentKeyCode = null; // 每次进入handleSelect框选方法，将_currentKeyCode置为null

      if(Autodesk.Viewing.inFullscreen()) { // 如果进入全屏，重新计算_container的偏移
        _container_bounding_top = _container.getBoundingClientRect().top;
        _container_bounding_left = _container.getBoundingClientRect().left;
      }    
    ...
    function handleEndSelect() {
      ...
      // 取消active
        //debugger;
        console.log(BimBdip.view.getToolbar().getControl(av.TOOLBAR.BASEMODELSHOWSID)._controls[8].container);
        BimBdip.view.getToolbar().getControl(av.TOOLBAR.BASEMODELSHOWSID)._controls[8].container.classList.remove("active");
    ...
    function onKeyUp(evt) {  
      ...
      if(evt.keyCode == 83){
        ...
        //lock navigation0 0to fix the camera
         _viewer.navigation.setIsLocked(true) ; 
    ...
    function onMouseMove(evt) {  
      ...
      if(_running){ 
        ...
        _mouseEnd.x = evt.clientX - _container_bounding_left;
        _mouseEnd.y =  evt.clientY - _container_bounding_top;
    ...
    function onMouseUp(evt) { 
      //var viewport = _viewer.impl.clientToViewport(evt.clientX, evt.clientY); 
      _viewer.canvas.classList.remove("crosshair")
      if(_running){
        //get mouse points 
        // _mouseEnd.x = evt.clientX;
        // _mouseEnd.y =  evt.clientY;
        ...
        //get intesect box with the area of select window
        var ids = compute({clientX:_mouseStart.x,clientY:_mouseStart.y},
          {clientX:_mouseEnd.x,clientY:_mouseEnd.y},
           false);
        var commenArr = null; // 数组相同部分
        var newIdsArr = null; // 增加、减少框选后新数组
        
        console.log("_currentKeyCode-->"+_currentKeyCode)
        switch(_currentKeyCode) { 
          case 16: // shfit减少框选
            commenArr =  _.intersection(ids, _glob_idsArr); // 数组交集
            newIdsArr = _.difference(_glob_idsArr, commenArr); // 数组中过滤指定元素

            _viewer.select(newIdsArr);
            _glob_idsArr = newIdsArr;
            break;
          case 17: // ctrl增加框选
            newIdsArr = _.union(_glob_idsArr, ids); // 数组合并，去除重复值
    ...
    function onMouseDown(evt) { 
      ...
      //get mouse points  
      _mouseStart.x = evt.clientX - _container_bounding_left;
      _mouseStart.y =  evt.clientY - _container_bounding_top;
      _running = true; 
    ...
    function compute (pointer1, pointer2,partialSelect) {
      ...
      var ray1 = pointerToRay({
        clientX: xMin + _container_bounding_left,
        clientY: yMin + _container_bounding_top
      })

      var ray2 = pointerToRay({
        clientX: xMax + _container_bounding_left,
        clientY: yMin + _container_bounding_top
      })

      var ray3 = pointerToRay({
        clientX: xMax + _container_bounding_left,
        clientY: yMax + _container_bounding_top
      })
